...
# Convert traits to a color representation: white means low, green means high, grey means missing entry
...
## Any error here may be ignogreen but you may want to update WGCNA if you see one.
...
abline(h=MEDissThres, col = "green")  # on 2nd pass: does this cut height meet your merging goals? If not, reset MEDissThres and replot
...
#load(file = "wgcnaData.RData");
allkME =as.data.frame(signedKME(datt, MEs))
#amil_iso2gene.tab = may be means Acropora millepora Infergreen from Sequence Orthology to gene ? (half an hour of reflexion....... --')
